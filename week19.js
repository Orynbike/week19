...
function makeTwelve() {
	const inputElement = document.getElementById('ipInput'); // Assuming you have an input field with id 'ipInput'
  
	const ipAddress = inputElement.value;
  
	fetch(`http://api.ipstack.com/${ipAddress}?access_key=YOUR_ACCESS_KEY`)
	  .then(response => response.json())
	  .then(data => {
...
function makeFourteen() {
	fetch('https://official-joke-api.appspot.com/random_joke')
	  .then(response => response.json())
	  .then(data => {
		const jokeElement = document.getElementById('jokeResult');
...
function makePromiseAllOne() {
	...
	Promise.all([
	  ...
	  fetch(urlComment).then(response => response.json()),
	])
	  .then(([post, comment]) => {
